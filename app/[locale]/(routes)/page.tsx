...
import { getWhatsAppMessagesToday } from "@/actions/dashboard/get-whatsapp-today";
import { getPendingFollowUps } from "@/actions/dashboard/get-pending-followups";
...
const DashboardPage = async () => {
  ...
  const dict = await getDictionary("pt"); //Fetch data for dashboard
  ...
  const whatsappToday = await getWhatsAppMessagesToday();
  ...
  return (
    <Container
      ...
      description="Bem-vindo ao painel Balako Digital CRM, aqui você pode ver a visão geral da sua empresa"
    >
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {/* Novo Widget: Mensagens WhatsApp Hoje */}
        <DashboardCard
          href="/crm/whatsapp"
          title="Mensagens WhatsApp (hoje)"
          IconComponent={MessageCircle}
          content={whatsappToday}
        />

        {/* Novo Widget: Follow-ups Pendentes */}
        <DashboardCard
          href="/projects/tasks"
          title="Follow-ups Pendentes"
          IconComponent={CheckSquare}
          content={pendingFollowUps}
        />

        {/* Widget: Total de Contatos */}
        <DashboardCard
          href="/crm/contacts"
